...
- **THEN** the Device section renders after the projects section and before the Bookshelf section

### Requirement: 3D PC model with spec sheet
The Device section SHALL render a 3D model of the PC (using the site's TresJS/Three.js stack) alongside a spec sheet listing every part of the build (CPU, GPU, memory, motherboard, storage, cooling, PSU, case).
...
- **THEN** the 3D PC model renders in a section-scoped canvas next to the full spec sheet
